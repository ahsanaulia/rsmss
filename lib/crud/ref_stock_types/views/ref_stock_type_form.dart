...
class RefStockTypeFormPage extends ConsumerStatefulWidget {
  final RefStockTypeModel? type;

  const RefStockTypeFormPage({
    super.key,
    this.type,
  });

  @override
  ConsumerState<RefStockTypeFormPage> createState() => _RefStockTypeFormPageState();
}

class _RefStockTypeFormPageState extends ConsumerState<RefStockTypeFormPage> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      final subCategoryState = ref.read(refStockSubCategoryProvider);
  ...
  Future<void> _showSubCategoryPicker(List<RefStockSubCategoryModel> subCategories) async {
    final TextEditingController searchController = TextEditingController();
    List<RefStockSubCategoryModel> filteredSubCategories = List.from(subCategories);
                    ...
                    Padding(
                      ...
                      child: TextField(
                        ...
                        decoration: InputDecoration(
                          hintText: 'Cari sub-kategori stok...',
  ...
  Future<void> _submitForm() async {
    ...
    if (_selectedSubCategoryId == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Pilih sub-kategori stok terlebih dahulu'), backgroundColor: Colors.red),
      );
      return;
    }

    final isEditing = widget.type != null;
    final type = RefStockTypeModel(
      id: widget.type?.id,
    ...
    final notifier = ref.read(refStockTypeProvider.notifier);
    bool success;
    ...
    if (success && mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(isEditing ? 'Tipe stok berhasil diupdate' : 'Tipe stok berhasil ditambahkan'),
  ...
  String? _validateName(String? value) {
    if (value == null || value.trim().isEmpty) {
      return 'Nama tipe stok wajib diisi';
    }
    if (value.trim().length < 2) {
      return 'Nama tipe stok minimal 2 karakter';
    }
    if (value.trim().length > 100) {
      return 'Nama tipe stok maksimal 100 karakter';
    }
    return null;
  }
  ...
  Widget build(BuildContext context) {
    ...
    final typeState = ref.watch(refStockTypeProvider);
    final isEditing = widget.type != null;
    ...
    final selectedSubCategoryDisplay = _selectedSubCategoryName ?? 
        (_selectedSubCategoryId != null 
            ? subCategories.firstWhere(
                ...
                orElse: () => RefStockSubCategoryModel.empty()
              ).subCategoryName 
            : null);

    return Scaffold(
      appBar: AppBar(
        title: Text(isEditing ? 'Edit Tipe Stok' : 'Tambah Tipe Stok'),
          ...
          children: [
                        ...
                        children: [
                          ...
                          Text(
                            isLoadingSubCategories 
                                ? 'Memuat sub-kategori...' 
                                : (selectedSubCategoryDisplay ?? 'Pilih sub-kategori stok'),
                            style: TextStyle(
                              fontSize: 16,
            ...
            TextFormField(
              ...
              decoration: const InputDecoration(
                labelText: 'Nama Tipe Stok *',
                border: OutlineInputBorder(),
                prefixIcon: Icon(Icons.label),
                hintText: 'Contoh: Obat Generik, Obat Paten, Alat Sekali Pakai',
              ),
            ...
            TextFormField(
              ...
              decoration: const InputDecoration(
                ...
                hintText: 'Deskripsi tipe stok (opsional)',
              ),
              maxLines: 3,
              enabled: !isSubmitting,
            ),
                    ...
                    Row(
                      children: [
                        ...
                        Column(
                          ...
                          children: [
                            Text(
                              _nameController.text.isEmpty ? 'Nama Tipe Stok' : _nameController.text,
  ...
  Widget _buildSubCategoryIcon(String? iconName) {
    ...
    if (iconData != null) {
      return Icon(iconData, size: 24, color: Colors.teal);
    }
    return const Icon(Icons.subdirectory_arrow_right, size: 24, color: Colors.teal);
  }
  ...
  Widget _buildPreviewIcon() {
    if (_selectedIcon == null) {
      return Container(
        ...
        child: const Icon(Icons.label, color: Colors.teal, size: 28),
      );
    }

...
import 'package:rsmss/crud/ref_asset_sub_categories/services/ref_asset_sub_category_service.dart';
import 'ref_asset_sub_category_state.dart';

final refAssetSubCategoryServiceProvider = Provider<RefAssetSubCategoryService>((ref) {
  return RefAssetSubCategoryService();
});

final refAssetSubCategoryProvider = StateNotifierProvider<RefAssetSubCategoryNotifier, RefAssetSubCategoryState>((ref) {
  final service = ref.watch(refAssetSubCategoryServiceProvider);
  return RefAssetSubCategoryNotifier(service);
});

class RefAssetSubCategoryNotifier extends StateNotifier<RefAssetSubCategoryState> {
  final RefAssetSubCategoryService _service;

  RefAssetSubCategoryNotifier(this._service) : super(RefAssetSubCategoryState.initial());
  ...
  Future<void> loadSubCategories() async {
    ...
    try {
      ...
      final subCategories = response.map((json) => RefAssetSubCategoryModel.fromJson(json)).toList();
  ...
  Future<void> loadSubCategoryById(String id) async {
    ...
    try {
      ...
      if (response != null) {
        final subCategory = RefAssetSubCategoryModel.fromJson(response);
        state = state.copyWith(
        ...
        state = state.copyWith(
          isLoading: false,
          errorMessage: 'Sub-kategori tidak ditemukan',
        );
      }
    } catch (e, stackTrace) {
  ...
  Future<bool> createSubCategory(RefAssetSubCategoryModel model) async {
    ...
    try {
      ...
      if (exists) {
        state = state.copyWith(
          isSubmitting: false,
          errorMessage: 'Nama sub-kategori "${model.subCategoryName}" sudah ada di kategori ini',
        );
        return false;
      }
  ...
  Future<bool> updateSubCategory(RefAssetSubCategoryModel model) async {
    debugPrint('✏️ [Provider] updateSubCategory - ID: ${model.id}, Name: ${model.subCategoryName}');
    state = state.copyWith(isSubmitting: true, errorMessage: null);

    try {
      if (model.id == null) {
        throw Exception('ID sub-kategori tidak ditemukan');
      }
  ...
  void setSelectedSubCategory(RefAssetSubCategoryModel? subCategory) {
